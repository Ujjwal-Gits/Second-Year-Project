...
const Hero = () => {
    ...
    const handleSearchChange = (e) => {
        ...
        if (value.length > 0) {
            setFilteredDestinations(destinations.filter(d => d.City.toLowerCase().startsWith(value.toLowerCase())));
        } else {
    ...
    return (
        <section className="relative w-full h-screen font-sans overflow-hidden scroll-snap-align-start">
            ...
            <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4 pt-24 md:pt-32 pb-32">
                ...
                <h1 className="text-white text-4xl md:text-6xl lg:text-7xl font-sans font-medium tracking-tight uppercase mb-12 drop-shadow-2xl opacity-95">
                    Amazing Mountain <br />
                    <span className="font-bold block mt-2">To Explore</span>
                </h1>

                {/* Simple Professional Search Bar */}
                <div className="w-full max-w-xl relative mb-auto" ref={searchRef}>
                    <div className="bg-white/10 backdrop-blur-md border border-white/20 p-2 rounded-full flex items-center shadow-2xl transition-all hover:bg-white/15">
                        <input 
                            ...
                            className="flex-1 bg-transparent border-none text-white placeholder-gray-300 px-6 py-3 focus:outline-none text-base md:text-lg font-light tracking-wide"
                            value={query}
                            onChange={handleSearchChange}
                        />
                        <button 
                            onClick={() => navigate('/explore')}
                            className="bg-[#0F5146] hover:bg-[#0A3D34] text-white px-8 py-3 md:px-10 md:py-4 rounded-full font-bold uppercase tracking-widest text-[10px] md:text-xs transition-colors duration-300 shadow-lg whitespace-nowrap"
                        >
                            Explore Now
                        </button>
                    </div>
                    ...
                    {showSuggestions && filteredDestinations.length > 0 && (
                        <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-xl shadow-xl overflow-hidden text-left z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                            {filteredDestinations.map((dest, i) => (
                                <div key={i} onClick={() => selectDestination(dest.City)} className="px-6 py-3 hover:bg-gray-50 cursor-pointer text-gray-800 transition-colors border-b border-gray-100 last:border-0 text-sm">
                                    <span className="font-bold">{dest.City}</span> <span className="text-gray-400 text-xs uppercase ml-2">{dest.Type}</span>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <div className="absolute bottom-0 left-0 w-full z-20 px-4 md:px-8 pb-8 md:pb-12">
                <div className="max-w-6xl mx-auto border-t border-white/15 pt-8">
                    ...
                    <div className="hidden md:flex flex-row justify-center items-center gap-16 text-white mt-[-2rem]"> {/* Adjusted neg margin to replace grid above if needed, but let's just replace the grid content */}
