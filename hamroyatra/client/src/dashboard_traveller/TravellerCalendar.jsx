...
const TravellerCalendar = ({ user }) => {
  ...
  const selectedDayPlans = selectedDay
    ? myPlans.filter((p) => p.startDate === selectedDateStr)
    : [];
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
      ...
      <div className="mb-4 md:mb-8">
        <h1 className="text-xl md:text-2xl font-bold text-[#1a202c]">
          Adventure Schedule
        </h1>
        <p className="text-[11px] md:text-sm text-gray-500 mt-1 font-medium">
          Temporal matrix of your upcoming plans and reservations.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        >
          ...
          <div className="p-4 md:p-6 flex items-center justify-between border-b border-gray-50">
            <h2 className="text-lg md:text-xl font-bold text-[#1a202c]">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={goToToday}
                className="px-5 py-1.5 border border-gray-200 rounded-lg text-sm font-bold text-[#1a202c] hover:bg-gray-50 transition-colors"
              >
                Today
              </button>
              <button
          ...
          <div className="grid grid-cols-7 border-b border-gray-100">
            {DAYS.map((d) => (
              <div
                key={d}
                className="py-3 text-center text-[10px] font-bold text-gray-400 tracking-[0.1em]"
              >
                {d}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7 transition-opacity">
            {cells.map((cell, i) => {
              ...
              return (
                ...
                >
                  <span
                    className={`text-xs md:text-base font-bold ${!cell.current ? "text-gray-200" : "text-[#2d3748]"} ${isTodayCell ? "text-primary" : ""}`}
                  >
                    {cell.day}
                  </span>
        ...
        <motion.div
          ...
          style={{
            height: window.innerWidth > 1024 ? "calc(100vh - 120px)" : "auto",
            maxHeight: "780px",
          }}
        >
          ...
          <div className="flex-1 p-4 space-y-3 overflow-y-auto">
            {!selectedDay ? (
              <div className="h-full flex flex-col items-center justify-center text-center opacity-40 py-10">
                <span className="material-icons text-3xl text-gray-300 mb-3">
                  touch_app
                </span>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center opacity-30 py-10">
                <span className="material-icons text-3xl text-gray-200 mb-3">
                  event_available
                </span>
                ...
                {selectedDayPlans.map((p, idx) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span
                        className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md text-white`}
                        style={{ backgroundColor: p.color }}
                      >
                        {p.type}
                      </span>
                    </div>
                    <p className="text-[12px] font-bold text-[#1a202c] leading-tight mb-2 uppercase">
                      {p.title}
                    </p>
