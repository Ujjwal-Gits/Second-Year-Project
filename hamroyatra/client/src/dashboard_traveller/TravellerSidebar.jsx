...
const navItems = [
    ...
    { id: 'calander', icon: 'calendar_month', label: 'My Calendar' },
    { id: 'bookings', icon: 'receipt_long', label: 'My Bookings' },
    { id: 'messages', icon: 'forum', label: 'Messages' },
    { id: 'reviews', icon: 'rate_review', label: 'My Reviews' },
    { id: 'history', icon: 'history', label: 'History' },
];

const TravellerSidebar = ({ activeTab, onTabChange, user, onClose, hasUnreadMessages }) => {
    const displayName = user?.fullName || 'Traveller';
        ...
        <aside className="w-64 lg:w-60 bg-[#0D1F18] flex flex-col h-full shrink-0 z-20 relative overflow-hidden">
            ...
            <div className="h-[72px] flex items-center justify-between px-6 border-b border-white/5 shrink-0 relative z-10">
                <div className="flex items-center">
                    ...
                    <div>
                        <p className="text-white text-[11px] font-black tracking-[0.25em] uppercase leading-none">Hamroyatra</p>
                        <p className="text-[#C5A059] text-[8px] tracking-[0.3em] uppercase opacity-60 mt-0.5">Traveller Portal</p>
                    </div>
                </div>
                <button 
                    onClick={onClose}
            ...
            <div className="px-6 pb-6 relative z-10">
                <div className="border-t border-white/5 pt-6 pb-2">
                    <button
                        onClick={() => navigate('/')}
                        className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-300 group hover:bg-white/5 border border-transparent hover:border-white/5"
                    >
