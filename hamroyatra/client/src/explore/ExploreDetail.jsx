...
const BookingCard = ({
  ...
  const allRooms = [
    {
      ...
      label: "AC Room",
      qty: listing.acRooms,
      price: listing.acPrice,
      icon: "ac_unit",
    },
    {
      key: "nonAc",
      label: "Non-AC Room",
      qty: listing.nonAcRooms,
      price: listing.nonAcPrice,
      icon: "hotel",
    },
    {
      key: "family",
      label: "Family Suite",
      qty: listing.familyRooms,
      price: listing.familyPrice,
      icon: "groups",
    },
    {
      key: "couple",
      label: "Couple Suite",
      qty: listing.coupleRooms,
      price: listing.couplePrice,
      icon: "favorite",
    },
  ].filter(
    (r) =>
      ...
      bookingForm.roomSelection[r.key] > 0,
  );
  ...
  const upd = (key, d, max) =>
    setBookingForm((b) => ({
      ...
      roomSelection: {
        ...b.roomSelection,
        [key]: Math.min(max || 99, Math.max(0, b.roomSelection[key] + d)),
      },
    }));
    ...
    >
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-[20px] font-black text-[#1A2B23] tracking-tight leading-none">
            NPR {calculateTotal().toLocaleString()}
          </p>
        </div>
        <div className="h-8 w-px bg-gray-100" />
        <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest bg-emerald-50 px-2.5 py-1 rounded-full">
          In Stock
        </span>
      </div>
      ...
      <form onSubmit={handleBooking} className="space-y-3">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  const isFixedDuration =
                    listing.type !== "hotel" && parseInt(listing.duration) > 0;
                  ...
                  if (isFixedDuration) {
                    ...
                    !newForm.endDate ||
                    new Date(newForm.endDate) <= start
                  ) {
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <input
                ...
                readOnly={
                  parseInt(listing.duration) > 0 && listing.type !== "hotel"
                }
                onChange={(e) =>
                  setBookingForm((b) => ({ ...b, endDate: e.target.value }))
                }
        ...
        {listing.type !== "hotel" ? (
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={() =>
                  setBookingForm((b) => ({
                    ...b,
                    roomCount: Math.max(1, b.roomCount - 1),
                  }))
                }
              ...
              <span className="text-[12px] font-black text-[#1A2B23] min-w-[18px] text-center">
                {bookingForm.roomCount}
              </span>
              <button
                type="button"
                onClick={() =>
                  setBookingForm((b) => ({ ...b, roomCount: b.roomCount + 1 }))
                }
          ...
          <div>
            <p className="text-[9px] font-black text-gray-500 uppercase tracking-[0.18em] mb-2 px-1">
              Select Rooms
            </p>
            <div className="grid grid-cols-2 gap-2">
              {allRooms.map((room) => {
                ...
                return (
                  <div
                    ...
                    className={`flex flex-col justify-between p-3 rounded-xl transition-all border ${
                      sel
                        ? "border-[#1A2B23]/15 bg-[#1A2B23]/[0.03]"
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <span
                        className={`text-[10px] font-black leading-tight ${sel ? "text-[#1A2B23]" : "text-gray-500"}`}
                      >
                        {room.label}
                      </span>
                      {sel && (
                    ...
                    <div className="flex items-center justify-between bg-white rounded-lg border border-gray-100 p-0.5">
                      <button
                        type="button"
                        onClick={() => upd(room.key, -1, room.qty)}
                      ...
                      <span className="text-[11px] font-black text-[#1A2B23]">
                        {bookingForm.roomSelection[room.key]}
                      </span>
                      <button
                        type="button"
        ...
        <div className="flex items-center gap-3 bg-[#F7F6F3] rounded-xl px-4 h-11 focus-within:ring-1 focus-within:ring-[#1A2B23]/10 transition-all">
          ...
          <input
            ...
            onChange={(e) =>
              setBookingForm((b) => ({ ...b, notes: e.target.value }))
            }
...
const ExploreDetail = ({ isAuthenticated, user }) => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/public/listings/${id}`,
        );
        setData(res.data);
        ...
        if (res.data?.listing?.companyName) {
          try {
            const reviewRes = await axios.get(
              `${import.meta.env.VITE_API_URL}/api/public/reviews?companyName=${encodeURIComponent(res.data.listing.companyName)}&listingId=${id}`,
            );
  ...
  const calculateTotal = () => {
    ...
    if (listing.type !== "hotel")
      return parseFloat(listing.price || 0) * (bookingForm.roomCount || 1);
  ...
  const handleBooking = async (e) => {
    ...
    if (!isAuthenticated) {
      setBookingStatus({
        type: "error",
        msg: "Please sign in to reserve this experience.",
      });
      return;
    }

    const { listing } = data;
    const totalRooms = Object.values(bookingForm.roomSelection).reduce(
      (a, b) => a + b,
      0,
    );
    if (listing.type === "hotel" && totalRooms === 0) {
      setBookingStatus({
        type: "error",
        msg: "Please select at least one room.",
      });
      return;
    }
    ...
    try {
      const payload = {
        ...
        guestPhone: user.phoneNo || "",
        roomCount:
          listing.type === "hotel" ? totalRooms : bookingForm.roomCount || 1,
        roomSelection:
          listing.type === "hotel" ? bookingForm.roomSelection : {},
        notes: bookingForm.notes || "",
      };

      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/dashboard/bookings`,
        payload,
      ...
      setBookingStatus({
        type: "error",
        msg:
          err.response?.data?.error || "Reservation failed. Please try later.",
      });
    }
  };
  ...
  const handleMessage = async (e) => {
    ...
    if (!isAuthenticated) {
      setMessageStatus({
        type: "error",
        msg: "Please sign in to contact the operator.",
      });
      return;
    }
    try {
      ...
      setMessageStatus({
        type: "success",
        msg: "Inquiry sent! The partner will reach out soon.",
      });
  ...
  const handleReview = async (e) => {
    ...
    if (!isAuthenticated) {
      setReviewStatus({
        type: "error",
        msg: "Please sign in to share your experience.",
      });
      return;
    }
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/dashboard/public/review`,
        {
          ...reviewForm,
          companyName: data.listing.companyName,
      ...
      try {
        const reviewRes = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/public/reviews?companyName=${encodeURIComponent(data.listing.companyName)}&listingId=${id}`,
        );
        setReviews(reviewRes.data || []);
      } catch (e) {}
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FAFAF8] gap-5">
        ...
        <button
          ...
          className="text-[#1A2B23] text-[10px] font-black uppercase tracking-[0.2em] hover:underline flex items-center gap-2"
        >
  ...
  return (
    <div className="bg-[#FAFAF8] min-h-screen">
      <SEO
        title={`${listing.title} | ${typeLabels[listing.type] || "Nepal Experience"}`}
        description={
          listing.description
            ? listing.description.slice(0, 155)
            : `Book ${listing.title} with ${company?.companyName || "a verified Nepal travel partner"} on HamroYatra.`
        }
        keywords={`${listing.title}, ${listing.type} Nepal, ${listing.location || "Nepal"} ${listing.type}, book ${listing.type} Nepal`}
        canonical={`/explore/${listing.id}`}
        ogImage={images[0]}
        ogType="product"
        schema={{
          "@context": "https://schema.org",
          "@type": listing.type === "hotel" ? "LodgingBusiness" : "TouristTrip",
          name: listing.title,
          description: listing.description || `${listing.title} in Nepal`,
          url: `https://hamroyatra.ujjwalrupakheti.com.np/explore/${listing.id}`,
          image: images[0],
          offers: {
            "@type": "Offer",
            price: listing.price,
          ...
          <div className="lg:col-span-7 xl:col-span-8">
              ...
              <div className="flex items-center gap-4 flex-wrap">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-[11px] font-bold text-[#1A2B23] block leading-tight hover:underline">
                      ...
                      {company?.verified && (
                        <span className="text-[8px] text-green-600 font-bold uppercase tracking-widest flex items-center gap-1">
                          Verified Partner{" "}
                          <span className="material-icons text-green-500 text-[10px]">
                            verified
                          </span>
                        </span>
                      )}
                ...
                <div className="flex items-center gap-1.5">
                  {avgRating ? (
                    <>
                      <span className="material-icons text-[#C5A059] text-sm">
                        star
                      </span>
                      <span className="text-sm font-black text-[#1A2B23]">
                        {avgRating}
                      </span>
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-6 mb-10 py-6 border-y border-gray-100/60">
              {[
                {
                  icon: "hotel",
                  label: typeLabels[listing.type] || listing.type,
                  sublabel: "Identity",
                },
                ...
                <div key={i} className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-[#F7F6F3] flex items-center justify-center">
                    <span className="material-icons text-base text-[#1A2B23] opacity-60">
                      {chip.icon}
                    </span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[7px] font-black text-[#C5A059] uppercase tracking-[0.2em] leading-none mb-1">
                      {chip.sublabel}
                    </span>
                    <span className="text-[11px] lg:text-[12px] font-black text-[#1A2B23] leading-tight tracking-tight truncate">
                      {chip.label}
                    </span>
                  </div>
                </div>
              ...
              <div className="mb-12">
                ...
                <div className="grid grid-cols-3 gap-2 md:gap-3">
                  {listing.amenities.map((amenity, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 px-2.5 py-3 bg-white border border-gray-100/60 rounded-xl shadow-sm hover:shadow-md transition-shadow"
                    >
                      <div className="w-5 h-5 rounded-lg bg-[#C5A059]/10 flex items-center justify-center shrink-0">
                        <span className="material-icons text-[#C5A059] text-[12px]">
                          check
                        </span>
                      </div>
                      <span className="text-[9px] md:text-[11px] font-bold text-[#1A2B23] tracking-tight leading-tight line-clamp-2">
                        {amenity}
                      </span>
                    </div>
                  ))}
            ...
            <div className="mb-14">
              ...
              <p className="text-gray-500 text-[15px] font-light leading-[2] whitespace-pre-wrap">
                {listing.description ||
                  "Detailed information about this experience will be provided by the partner company. Contact the operator for more details about this experience."}
              </p>
            </div>

            {/* Itinerary Section */}
            {(listing.type === "trekking" || listing.type === "travel") &&
              listing.itinerary &&
                      ...
                      >
                        ...
                        <div className="p-6 md:p-8 bg-white rounded-2xl border border-gray-100/80 shadow-sm group-hover:shadow-md transition-all group-hover:border-[#C5A059]/10">
                          <p className="text-gray-500 text-[13px] font-medium leading-[1.8] whitespace-pre-wrap">
                            <span className="text-[10px] font-black text-[#C5A059] uppercase tracking-[0.2em] mr-4 inline-block">
                              Day {idx + 1}:
                            </span>
                            {desc}
                          </p>
              ...
              <div className="mb-12">
                ...
                <div className="bg-gradient-to-br from-[#C5A059]/[0.06] to-[#C5A059]/[0.02] rounded-2xl p-7 border border-[#C5A059]/10">
                  <div className="flex items-start gap-3">
                    <span className="material-icons text-[#C5A059] text-xl mt-0.5">
                      local_offer
                    </span>
            ...
            <div className="mb-12">
              ...
              {reviews.length > 0 ? (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-[12px] font-bold text-[#1A2B23] block">
                        ...
                        <div className="flex gap-0.5">
                          {[...Array(5)].map((_, s) => (
                            <span
                              key={s}
                              className={`material-icons text-[13px] ${s < (review.rating || 0) ? "text-[#C5A059]" : "text-gray-200"}`}
                            >
                              star
                            </span>
                          ))}
                ...
                <div className="bg-white rounded-2xl p-8 border border-gray-100/80 text-center mb-8">
                  <span className="material-icons text-gray-200 text-3xl mb-2 block">
                    rate_review
                  </span>
              ...
              <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-100/80">
                <h4 className="text-[11px] font-black text-[#1A2B23] uppercase tracking-[0.15em] mb-5">
                  Write a Review
                </h4>
                <form onSubmit={handleReview} className="space-y-4">
                  {!isAuthenticated ? (
                    ...
                    <>
                      <div>
                        <label className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">
                          Rating
                        </label>
                      ...
                      <div className="flex items-center justify-between pb-2">
                        <span className="text-[9px] text-gray-400 font-medium italic">
                          Posting as{" "}
                          <span className="text-[#1A2B23] font-bold not-italic">
                      ...
                      <button
                        type="submit"
                        className="w-full h-12 bg-[#1A2B23] text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#2D4F3C] transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                      >
                        Submit Review
            ...
            <div className="bg-white rounded-2xl p-7 md:p-8 border border-gray-100/80 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.06)]">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-xl bg-[#F7F6F3] flex items-center justify-center">
                  <span className="material-icons text-[#1A2B23] text-lg">
                    mail_outline
                  </span>
                </div>
                <div>
                  <h3 className="text-[12px] font-black text-[#1A2B23]">
                    Contact Operator
                  </h3>
              ...
              <form onSubmit={handleMessage} className="space-y-4">
                {!isAuthenticated ? (
                  <div className="py-4 text-center">
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-3 leading-relaxed">
                      Please sign in to message the operator directly
                    </p>
                    <button
                      type="button"
                      onClick={() => navigate("/login")}
                  ...
                  <>
                    ...
                    <button
                      type="submit"
                      className="w-full h-12 bg-[#1A2B23] text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-[#2D4F3C] transition-all active:scale-[0.98] flex items-center justify-center gap-2.5 cursor-pointer shadow-lg shadow-[#1A2B23]/5"
                    >
                      Send Inquiry
                      <span className="material-icons text-sm">send</span>
                    </button>
      ...
      <AnimatePresence>
        {showMobileBar && (
          ...
          >
            <div className="flex flex-col">
              <span className="text-[9px] font-black text-[#C5A059] uppercase tracking-widest block mb-0.5">
                Total Value
              </span>
              <div className="flex items-baseline gap-1.5">
                <span className="text-xl font-black text-[#1A2B23]">
            ...
            <button
              onClick={() =>
                document
                  .getElementById("reservation-card")
